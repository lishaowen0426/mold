...
namespace mold
{
                                        ...
                                        std::string archive_name)
  {
    ...
    ObjectFile<E> *file =
        new ObjectFile<E>(ctx, mf, archive_name, in_lib, rctx.isolate);
    ctx.obj_pool.emplace_back(file);
    file->priority = ctx.file_priority++;

    rctx.tg->run([file, &ctx]
                 { file->parse(ctx); });
  ...
  void read_file(Context<E> &ctx, ReaderContext &rctx, MappedFile *mf)
  {
    ...
    {
      ObjectFile<E> *obj = new_object_file(ctx, rctx, mf, "");
      ctx.objs.push_back(obj);
      if (rctx.isolate)
      {
        SPDLOG_DEBUG("isolate object {} is recorded in ctx", obj->get_source_name());
        ctx.isolates.push_back(obj);
      }
      return;
    }
  ...
  int mold_main(int argc, char **argv)
  {
    ...
    SPDLOG_DEBUG("print file_args: \n");
    for (auto file = file_args.begin(); file != file_args.end(); file++)
    {
      SPDLOG_DEBUG("{}", *file);
    }
    ...
    std::erase_if(ctx.objs, [](InputFile<E> *file)
                  { return !file->is_alive; });
    std::erase_if(ctx.dsos, [](InputFile<E> *file)
                  { return !file->is_alive; });
    ...
    if (ctx.arg.relocatable)
    {
      SPDLOG_ERROR("arg.relocatable is set");
      exit(1);
      combine_objects(ctx);
      return 0;
    }
