...
namespace mold
{
  ...
  static void mark_live_objects(Context<E> &ctx)
  {
    ...
    tbb::parallel_for_each(roots, [&](InputFile<E> *file,
                           ...
                           { file->mark_live_objects(ctx, [&](InputFile<E> *obj)
                                                     { feeder.add(obj); }); });
  }
  ...
  void do_lto(Context<E> &ctx)
  {
    ...
    std::erase_if(ctx.objs, [](ObjectFile<E> *file)
                  { return file->is_lto_obj; });
  ...
  void create_output_sections(Context<E> &ctx)
  {
    ...
    for (std::unique_ptr<InputSection<E>> &isec : file->sections) {
      ...
      if (file->isolate){
        SPDLOG_DEBUG("input section: {} -> output section: {}", isec->name(), osec->name);
